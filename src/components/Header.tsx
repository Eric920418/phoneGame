  ...
  const navItems: NavItem[] = [
    {
      ...
      icon: Crown,
      dropdown: [
        { href: "/guide/settings", label: "遊戲設定", icon: Settings },
        { href: "/guide/beginner", label: "新手攻略", icon: BookOpen },
        { href: "/guide/drops", label: "掉落查詢", icon: Search },
        { href: "/guide/dungeon", label: "副本介紹", icon: Map },
        { href: "/guide/treasure", label: "寶箱福袋", icon: Gift },
        { href: "/guide/nation-war", label: "國戰時間", icon: Swords },
        { href: "/guide/download", label: "下載專區", icon: Download },
      ]
    },
    {
      href: "/announcements",
                    ...
                    <div className="py-2 bg-[var(--color-bg-darker)] border border-[var(--color-border)] rounded-lg shadow-xl animate-fadeIn">
                      {item.dropdown.map((dropItem) => (
                        <Link
                          key={dropItem.href}
                          href={dropItem.href}
                          className="flex items-center gap-3 px-4 py-2.5 text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-bg-card)] transition-all whitespace-nowrap"
